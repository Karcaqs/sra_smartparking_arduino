...
void readNFC() {
  ...
  if (nfc.inListPassiveTarget()) {
    ...
    // Baca kartu ISO14443Ad
    success = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 500); //timeout 100ms
    //    success = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength);

    if (success) {
      //      Serial.println(F("Ditemukan kartu ISO14443A"));
      //      Serial.print(F("  Panjang UID: ")); Serial.print(uidLength, DEC); Serial.println(F(" bytes"));
      //      Serial.print(F("  Nilai UID: "));
      ...
      if (uidLength == 4) {
        ...
        cardid <<= 8;
        cardid |= uid[3];
        //        Serial.print(F("Terdeteksi Mifare Classic, ID: "));
        //        Serial.println(cardid);
      } else {
      ...
      //      Serial.print(F("UID sebagai string: "));
  ...
  //  else {
  //    Serial.println(F("TEMPELKAN KARTU!!!"));
  //  }
}
